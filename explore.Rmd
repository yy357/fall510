...
if (length(varnum) > 0){ #if there are numeric columns
  for (i in 1:length(varnum)){ #for loop, go through each column in the data frame
    for (bins in 1:length(bin_sizes)){ #for loop, through each bin size in the vector
      ...
      Num_Plot <- Num_Plot + geom_histogram(colour='blue', fill='blue', binwidth=numbins) + labs(x=names(varnum)[[i]]) + geom_vline(xintercept=col_mean, colour='red') #define the histogram. The color of the bars is made to be blue and the binwidth is made to num_bins. The label for the x axis is made to be the name of the ith numeric column. A red vertical line for the mean is added.
